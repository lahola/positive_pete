...
-(void)initPete
{
    ...
    pete.body = [physics createBodyAtLocation:ccp((pete.position.x - xMargin)/iPadConstant,(pete.position.y - yMargin)/iPadConstant)
                                    forSprite:pete friction:1.0f restitution:0.0f density:1.0 isBox:NO 
...
-(void)update:(ccTime)dt 
{
    ...
    //implenting fixed time step
    
    progress += dt;
    
    while (progress > .01f) {
        world->Step(.1f, velocityIterations, positionIterations);
        [self applyChargeForces:.01f];
        progress -= .01f;
    }
...
-(void)resetPete
{
    ...
    pete.body->SetAngularVelocity(0.0f);
                ...
                for (NSValue *bodyPointer in bodiesClicked) {
                    ...
                    if (GameObject *obj = (GameObject*)clickedBody->GetUserData()) {
                        if (obj.gameObjectType == kPositivePoleType || obj.gameObjectType == kNegativePoleType ||
                            obj.gameObjectType == kNeutralPadType || obj.gameObjectType == kReversPadType ||
                            obj.gameObjectType == kSpeedPadType) {
                            objectTouched = obj;
            ...
            if ([objectTouched gameObjectType] == kCannonType) {
                ...
                switch (gameState) {
                    ...
                    default:
                       ...
                       [objectTouched gameObjectType] == kSpeedPadType ||
                       [objectTouched gameObjectType] == kNeutralPadType ||
                       [objectTouched gameObjectType] == kReversPadType) {
                switch (gameState) {
                    case kSetupPhaseState:
...
-(void) ccTouchMoved:(UITouch *)touch withEvent:(UIEvent *)event {
    ...
    if (selectedBody) {
        ...
        if (self.gameState == kMovingPhaseState) {
            [self resetPete];
        }
    }
}
