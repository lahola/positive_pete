...
-(void)changeState:(CharacterState)newState {
    ...
    switch (newState) {
        case kStateReady:
            ...
            action = [CCAnimate actionWithDuration:1.8 animation:readyAnim 
                               restoreOriginalFrame:NO];
            break;
            
        case kStateFiring:
            CCLOG(@"Cannon is firing");
            action = [CCAnimate actionWithDuration:1.8 animation:firingAnim 
                               restoreOriginalFrame:NO];
            break;
            
        case kStateFired:
            CCLOG(@"Cannon has fired");
            action = [CCAnimate actionWithDuration:1.8 animation:firedAnim 
                               restoreOriginalFrame:NO];
            break;
